...
data "azurerm_key_vault_key" "cbd_global_bastion_ssh_key" {
...
resource "azurerm_linux_virtual_machine" "cbd_global_bastion_vm" {
  ...
  admin_ssh_key {
    username   = "adminuser"
    public_key = data.azurerm_key_vault_key.cbd_global_bastion_ssh_key.value
  }
...
data "azurerm_public_ip" cbd_global_bastion_ip_data" {
  name                = azurerm_public_ip.cbd_global_bastion_ip.name
  resource_group_name = azurerm_resource_group.cbd_global_rg.name
}

output "public_ip_address" {
  value = "${azurerm_linux_virtual_machine.cbd_global_bastion_vm.name}: ${data.azurerm_public_ip.cbd_global_bastion_ip_data.ip_address}"
}